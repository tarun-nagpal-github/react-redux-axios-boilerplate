  ...
  login = (event) => {
    ...
    axios.post('http://192.168.9.129/api/Auth/login', data, {
      ...
      if (response.status == 200) {
        ...
        axios.get('http://192.168.9.129:500/api/access/permissions', { // api/access/permissions
          headers
        }).then((response) => {

          window.localStorage.setItem('moduleAndPages', JSON.stringify(response.data.data));
          this.setState({
            redirectTo: true
          });
        }).catch((error) => {
              ...
              <Col lg={4} md={6} className="bg-white">
                <form onSubmit={this.login}>
                  <div className="login-fancy pb-40 clearfix">
                    ...
                    {/* <a type="submit" className={`btn btn-primary   ${this.isFormValid() ? '' : 'disabled'}`}>
                      <span className="text-white">Log in</span>
                      <i className="fa fa-check text-white" />
                    </a> */}

                    <button type="submit" className={`btn btn-primary   ${this.isFormValid() ? '' : 'disabled'}`}>
                      <span className="text-white">Log in</span>
                      <i className="fa fa-check text-white" />
                    </button>
